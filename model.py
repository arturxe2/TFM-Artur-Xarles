...
class Model(nn.Module):
    def __init__(self, weights=None, input_size=512, num_classes=3, vocab_size=128, chunk_size=240, framerate=2, pool="NetVLAD"):
        ...
        if self.pool == "MAX":
            ...
            encoder_layerB1 = nn.TransformerEncoderLayer(d_model=256, nhead=8)
            self.encoderB1 = nn.TransformerEncoder(encoder_layerB1, 1)
            encoder_layerB2 = nn.TransformerEncoderLayer(d_model=256, nhead=8)
            self.encoderB2 = nn.TransformerEncoder(encoder_layerB2, 1)
            encoder_layerB3 = nn.TransformerEncoderLayer(d_model=256, nhead=8)
            self.encoderB3 = nn.TransformerEncoder(encoder_layerB3, 1)
            encoder_layerB4 = nn.TransformerEncoderLayer(d_model=256, nhead=8)
            ...
            self.fcB1 = nn.Linear(256, self.num_classes+1)
            self.pool_layerB2 = nn.MaxPool1d(chunk_size, stride=1)
            self.fcB2 = nn.Linear(256, self.num_classes+1)
            self.pool_layerB3 = nn.MaxPool1d(chunk_size, stride=1)
            self.fcB3 = nn.Linear(256, self.num_classes+1)
            self.pool_layerB4 = nn.MaxPool1d(chunk_size, stride=1)
            self.fcB4 = nn.Linear(256, self.num_classes+1)
            self.pool_layerB5 = nn.MaxPool1d(chunk_size, stride=1)
            self.fcB5 = nn.Linear(256, self.num_classes+1)
            ...
            self.fc_mix = nn.Linear(256, self.num_classes+1)
